...
export function computePricing(input: PricingInput): PricingBreakdown {
  ...
  const hasModifiers = (input.adjustments?.length ?? 0) > 0
    || (input.taxes?.length ?? 0) > 0
  ...
  for (const a of input.adjustments ?? []) {
    const raw = typeof a.percent === 'number'
      ? (chargeSubtotal * a.percent) / 100
      : (a.amount ?? 0);
  ...
  for (const t of input.taxes ?? []) {
    ...
    rows.push({ id: t.id, label: t.label, detail: `${t.percent}% of ${formatMoney(net, input)}`, amount, role: 'tax' });
  }
  ...
  const payable = roundMoney(
    rows.filter((r) => r.role !== 'subtotal').reduce((s, r) => s + r.amount, 0),
  );
