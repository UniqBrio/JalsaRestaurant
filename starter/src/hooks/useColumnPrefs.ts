...
export function useColumnPrefs(columns: ColumnDef[], storageKey: string): ColumnPrefs {
  ...
  // Hydrate after mount, never during render: reading storage during render makes the render
  // impure, and on a server-rendered page there is no storage to read at all.
  useEffect(() => {
  ...
  }, [storageKey, allKeys, requiredKeys]);
