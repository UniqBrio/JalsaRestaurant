...
export function LoadingScreen({
  ...
  return (
    ...
    >
      ...
      {state.detail && (
        <p className="loading-screen__detail" data-testid={`${testId}-detail`}>{state.detail}</p>
      )}
      ...
      {state.showEscape && (
        <div className="loading-screen__escape" data-testid={`${testId}-escape`}>
          {onRetry && (
            <button type="button" className="loading-screen__retry" data-testid={`${testId}-retry`} onClick={onRetry}>
              Try again
            </button>
          )}
          {onLeave && (
            <button type="button" className="loading-screen__leave" data-testid={`${testId}-leave`} onClick={onLeave}>
              {leaveLabel}
            </button>
          )}
        </div>
