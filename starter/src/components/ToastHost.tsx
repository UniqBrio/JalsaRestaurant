...
import {
  createUndoState, dismissToast, expireToasts, pushToast, undoToast,
  type LiveToast, type UndoableEntry, type UndoState,
} from '../lib/undo';
...
export function ToastHost({
  state, onUndo, onDismiss, testId = 'toast',
}: {
  state: UndoState;
  ...
  return (
    <div className="toast-host" role="status" aria-live="polite" data-testid={testId}>
      {state.toasts.map((t) => (
        ...
        >
          <span className="toast__message" data-testid={`${testId}-${t.id}-message`}>{t.message}</span>
          {t.undo && (
            <button
              type="button"
