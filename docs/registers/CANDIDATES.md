...
|---|---|---|---|---|
| CAND-002 | "A guard that reads a named artifact must find that artifact wherever the change lives. A repository holding an application in a subdirectory has its gate log there, and a guard that greps only the root reports MISSING for evidence that exists - which trains people to reach for the escape token while doing exactly the right thing." | jalsa · 10-Sep-2026; again 11-Sep-2026 (Track R review, same repo, different commit) | n=2 | **ELIGIBLE** - second sighting recorded 11-Sep-2026. Still n=2 in ONE repository, which is the weaker kind of n=2: the register's rule is two *different apps*. Route through `workflows/promote.md` before promoting, and expect it to ask whether a monorepo counts as one app or two. The fix is small (`git diff --cached -- '*TEST_SUMMARY.md'` in `staged_diff`, or search every directory containing a `package.json`), but so is the blast radius of getting it wrong: a guard that matches too loosely satisfies itself from an unrelated file. Guard G3, `scripts/hooks/pre-commit-guard.sh:113`. |
