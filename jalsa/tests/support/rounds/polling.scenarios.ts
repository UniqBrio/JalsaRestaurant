...
const GRANTS: Record<string, string[]> = { st1: ['orders.view', 'tables.view'], ow1: ['orders.view'] };

const world = { floor: 41, catalog: 3, billVersion: 7, migrated: true };
...
function respond(q: FakeQuery): unknown[] | Record<string, unknown> | null {
  ...
  switch (q.table) {
    ...
    case 'guest_session':
      return q.op === 'select' ? [{ id: 's1', table_id: 't1', bill_id: null, heard_about: '' }] : [];
    case 'setting':
      return [];
    case 'restaurant':
