  ...
  if (kind === 'help') {
    ...
    return (
      ...
      >
            ...
            <li key={a.label}>
              ...
              >
                <span className="min-w-0">
                  ...
                  <span className="block text-[11.5px] text-[var(--text-muted)]">{a.note}</span>
                </span>
        ...
        <p className="m-0 mt-4 text-[11px] leading-relaxed text-[var(--text-muted)]">
    ...
    return (
      <Sheet
        ...
        footer={
          item.available ? (
            ...
            <p className="m-0 text-[12.5px] font-semibold text-[var(--text-muted)]">
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <p className="m-0 text-[13px] leading-relaxed text-[var(--text-muted)]">{item.description}</p>
          <p className="m-0 text-[12px] text-[var(--text-muted)]">
            {FOOD_TYPE[item.foodType].label} · {item.category}
          </p>
          <div className="flex items-center justify-between">
            <span className="text-[17px] font-bold">{item.priceLabel}</span>
            {item.available ? (
              <Stepper
                qty={itemQty}
      ...
      >
        <ul className="m-0 flex list-none flex-col gap-1.5 p-0">
          {data.hoursRows.map((h) => (
            <li
              key={h.day}
              className={`flex items-center justify-between rounded-[var(--radius-sm)] px-3 py-2 text-[12.5px] ${
                h.today
        ...
        {data.holidayNote ? (
          <p className="m-0 mt-3 text-[11.5px] leading-relaxed text-[var(--text-muted)]">{data.holidayNote}</p>
        ) : null}
        {data.callNumber ? (
  ...
  if (kind === 'occasion') {
    return (
      ...
      >
        ...
        <div className="mt-3">
          <label htmlFor="guest-occ-name" className="text-[12px] font-semibold">
            Whose is it?
          </label>
          <Input
            id="guest-occ-name"
        ...
        <p className="m-0 mt-3 text-[11px] leading-relaxed text-[var(--text-muted)]">
    ...
    return (
      ...
      >
        <Card className="bg-[var(--primary-surface)]">
          <p className="m-0 text-[13px] leading-relaxed text-[var(--on-primary-surface)]">
            {(
