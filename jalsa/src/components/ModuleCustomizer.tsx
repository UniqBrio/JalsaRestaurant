...
import { type ModuleItem, moveItem, positionBadges, setEnabled } from '../lib/module-customizer';
    ...
    return (
      <li key={it.id} className={`customize__row customize__row--depth-${depth}`}>
        ...
        {it.enabled && it.children && it.children.length > 0 && (
          <ul className="customize__children">{it.children.map((c) => renderRow(c, it.children!, depth + 1))}</ul>
        )}
      </li>
    );
  };
