...
export function PricingPanel({
  ...
  return (
    <section className="pricing" aria-labelledby={`${testId}-title`} data-testid={testId}>
      <h2 className="pricing__title" id={`${testId}-title`}>
        {title}
      </h2>
        ...
        <table className="pricing__table">
          ...
          <thead>
            <tr>
              ...
              <th scope="col" className="pricing__amount-head">
                Amount
              </th>
            </tr>
          </thead>
          <tbody>
            {result.rows.map((r) => (
              ...
              >
                <th scope="row" className="pricing__label">
                  {r.label}
                </th>
                <td className="pricing__detail">{r.detail ?? ''}</td>
                <td className="pricing__amount" data-testid={`${testId}-amount-${r.id}`}>
                  {money(r.amount)}
                </td>
              </tr>
            ))}
      ...
      {result.passThroughTotal !== 0 && (
        <p className="pricing__note" data-testid={`${testId}-passthrough-note`}>
          {money(result.passThroughTotal)} of this is collected on behalf of someone else and is not income.
        </p>
      )}

      {result.warnings.map((w, i) => (
        <p className="pricing__warning" key={w} data-testid={`${testId}-warning-${i}`}>
          {w}
        </p>
      ))}

      {footnote && (
        <p className="pricing__footnote" data-testid={`${testId}-footnote`}>
          {footnote}
        </p>
      )}
