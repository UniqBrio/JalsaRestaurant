    ...
    <div className="dash" data-testid={testId}>
      <header className="dash__header">
        ...
        <div className="dash__actions">
          {updatedAt && (
            <span className="dash__updated" data-testid={`${testId}-updated`}>
              Updated {updatedAt}
            </span>
          )}
          {onRefresh && (
            <button data-testid={`${testId}-refresh`} type="button" className="dash__action" onClick={onRefresh}>
              Refresh
            </button>
          )}
          {onExport && (
            <button data-testid={`${testId}-export`} type="button" className="dash__action" onClick={onExport}>
              Export
            </button>
          )}
          {onCustomize && (
            <button data-testid={`${testId}-customize`} type="button" className="dash__action" onClick={onCustomize}>
              Customise
            </button>
          )}
        </div>
      ...
      {config.sections.map((section) => (
        ...
        >
          {section.title && (
            <h2 className="dash__section-title" id={`${testId}-${section.id}-title`}>
              {section.title}
            </h2>
          )}
          ...
          <div className={section.kind === 'metrics' ? 'dash__metrics' : 'dash__body'}>{renderSection(section)}</div>
        </section>
      ))}
    </div>
  );
