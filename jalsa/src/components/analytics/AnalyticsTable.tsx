...
export function AnalyticsTable<T extends Row>({
  ...
  if (error) {
    return (
      <p className="atable__error" data-testid={`${testId}-error`}>
        {error}
      </p>
    );
  }
  ...
  return (
    <div className="atable" data-testid={testId}>
      ...
      {loading ? (
        <p className="atable__loading" aria-busy="true" data-testid={`${testId}-loading`}>
          Loading…
        </p>
      ) : list.result.rows.length === 0 ? (
        <p className="atable__empty" data-testid={`${testId}-empty`}>
          {emptyMessage}
        </p>
      ) : (
        <>
          ...
          <div className="atable__scroll">
            <table className="atable__table">
              <thead>
                <tr>
                  {columns.map((c) => (
                    <th key={c.key} scope="col" className={`atable__th atable__th--${c.align ?? 'start'}`}>
                      {c.label}
                    </th>
                  ))}
                  {onRowAction && (
                    <th scope="col" className="atable__th">
                      <span className="atable__sr">Actions</span>
                    </th>
                  )}
                </tr>
              </thead>
                ...
                {list.result.rows.map((row) => {
                  ...
                  return (
                    <tr key={id} className="atable__tr" data-testid={`${testId}-row-${id}`}>
                      {columns.map((c) => (
                        <td key={c.key} className={`atable__td atable__td--${c.align ?? 'start'}`}>
                          {cell(row, c)}
                        </td>
                      ))}
                      {onRowAction && (
