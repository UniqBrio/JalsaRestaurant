...
const SORTABLE = COLUMNS.filter((c) => !['previousValue', 'newValue', 'remarks'].includes(c.key)).map((c) => ({
  key: c.key,
  label: c.label,
}));
...
export function AuditLogTable({
  ...
  const moduleOptions = useMemo(() => modulesIn(entries).map((m) => ({ value: m, label: m })), [entries]);
