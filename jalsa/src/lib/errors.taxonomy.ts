...
export function normalize(err: unknown): NormalizedError {
  ...
  return {
    ...
    message: (
      pick('message', 'error_description', 'error', 'msg', 'details', 'hint') || (typeof err === 'string' ? err : '')
    ).toLowerCase(),
    name: pick('name'),
  };
}
...
export function classifyError(err: unknown): ErrorClass {
  ...
  if (
    has(message, ['refresh token not found', 'refresh token revoked', 'invalid refresh token', 'session not found'])
  )
    return 'session-dead';
  ...
  if (
    status === 403 ||
    code === '42501' ||
    has(message, ['permission denied', 'row-level security', 'not authorized', 'not authorised', 'forbidden'])
  )
    return 'forbidden';
  ...
  if (
    status === 409 ||
    code === '23505' ||
    has(message, ['duplicate key', 'already exists', 'unique constraint', 'version mismatch'])
  )
    return 'conflict';
  ...
  if (
    code === 'PGRST202' ||
    has(message, ['could not find the function', 'schema cache', 'unknown field', 'unsupported api version'])
  )
    return 'stale-client';

  if (status === 404 || code === '404' || has(message, ['not found', 'no rows returned'])) return 'not-found';
  if (status === 422 || status === 400 || has(message, ['validation', 'invalid input', 'is required', 'must be']))
    return 'validation';
  ...
  if (
    status === 401 ||
    code === 'PGRST303' ||
    has(message, ['jwt expired', 'jwt is expired', 'token expired', 'unauthenticated'])
  )
  ...
  if (name.toLowerCase() === 'aborterror' || has(message, ['operation was aborted', 'signal is aborted']))
    return 'network';
  if (name.toLowerCase() === 'typeerror' && has(message, ['fetch'])) return 'network';
  if (has(message, ['network', 'econnreset', 'etimedout', 'timeout', 'socket hang up', 'failed to fetch']))
    return 'network';
