...
const guardAlwaysOn = (item: ModuleItem, enabled: boolean): boolean => (item.alwaysOn ? true : enabled);
...
export function moveItem(items: readonly ModuleItem[], id: string, dir: 'up' | 'down'): ModuleItem[] {
  ...
  if (i >= 0) {
    ...
    const a = next[i];
    const b = next[j];
    // Both indices were bounds-checked above; the guard is what makes that legible to the
    // compiler rather than only to the reader.
    if (a === undefined || b === undefined) return items as ModuleItem[];
    next[i] = b;
    next[j] = a;
    return next;
  }
