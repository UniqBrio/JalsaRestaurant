...
export function auditAccountCreated(
  ...
  return auditEntry('Account created', null, account.name || account.email || account.id, {
    ...ctx,
    remarks: ctx.remarks ?? `Account id ${account.id}`,
  });
}
...
export function auditFeatureAccessChanged(
  ...
  return auditEntry('Feature access changed', [...previousFeatures], [...newFeatures], {
    ...ctx,
    ...(ctx.remarks !== undefined
      ? { remarks: ctx.remarks }
      : summary
        ? { remarks: summary.charAt(0).toUpperCase() + summary.slice(1) }
        : {}),
  });
}
