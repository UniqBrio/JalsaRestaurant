...
export function useListControls<T extends Row>(rows: readonly T[], config: ListConfig) {
  // `date` is optional on ListState, so a list with no date field carries NO date key rather
  // than an explicit undefined - which is what an "all dates" filter would otherwise look like.
  ...
  return {
    ...
    clearAll: () =>
      setState({ ...emptyListState(), ...(config.dateField ? { date: { preset: 'all' as const } } : {}) }),
  };
}
