...
export function useAsync<T>(fn: () => Promise<T>, context: string, deps: unknown[] = []) {
  ...
  const run = useCallback(async () => {
    ...
    // `deps` is a PARAMETER, so neither the exhaustive-deps rule nor the compiler can verify
    // it here - that verification belongs at each call site, where the dependencies are literals.
    // Both rules are silenced deliberately rather than by an inline array that would be wrong.
