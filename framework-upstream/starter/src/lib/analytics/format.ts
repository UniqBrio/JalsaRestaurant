...
export type FormatKind =
  | 'number' | 'decimal' | 'currency' | 'percent' | 'compact'
  | 'duration' | 'date' | 'datetime' | 'count';
...
export function formatValue(value: unknown, kind: FormatKind = 'number', opts: FormatOptions = {}): string {
  ...
  if (kind === 'date' || kind === 'datetime') {
    ...
    try { return new Intl.DateTimeFormat(locale, o).format(d); } catch { return d.toISOString().slice(0, 10); }
  }
  ...
  switch (kind) {
    case 'currency': {
      ...
      if (opts.compactStyle && Math.abs(n) >= (opts.compactStyle === 'in' ? 1e5 : 1e6)) {
        const symbol = intl(0, locale, { style: 'currency', currency, maximumFractionDigits: 0 }).replace(/[\d\s.,]/g, '');
        return `${sign}${symbol}${formatCompact(n, opts)}`;
      }
      return sign + intl(n, locale, {
        style: 'currency', currency,
        minimumFractionDigits: opts.decimals ?? 0, maximumFractionDigits: opts.decimals ?? 0,
      });
    }
    case 'percent':
