...
export function Sparkline({
  ...
  if (points.length < 2) {
    ...
    return <span className="sparkline sparkline--empty" data-testid={`${testId}-empty`}>—</span>;
  }
  ...
  const d = points
    .map((v, i) => `${i === 0 ? 'M' : 'L'}${(i * stepX).toFixed(2)},${(pad + usable - ((v - min) / span) * usable).toFixed(2)}`)
    .join(' ');
  ...
  return (
    <svg
      ...
      data-testid={testId}
    >
      <path className="sparkline__line" d={d} fill="none" stroke="currentColor" strokeWidth="1.5"
            strokeLinecap="round" strokeLinejoin="round" />
      <circle className="sparkline__end" cx={width} cy={pad + usable - ((last - min) / span) * usable} r="2"
              fill="currentColor" />
    </svg>
  );
}
