...
|---|---|---|---|---|---|
| FW-INTAKE-001 | Rough "does not exist yet" ask classifies NEW | `/request` with a description of a capability the codebase lacks | REQUEST_NEW filled; file written to `requests/<date>-<slug>.md`; run stops with "Review the FIELDS…" | The track starting; any field invented beyond the requester's words | 2026-09-04 |
...
| FW-INTAKE-004 | List / open situation / process failure route out with NO file | Three `/request` runs: a 5-item list; a "what should happen when…" situation; "the gate never caught it" | Routed to `/triage`, `/brainstorm`, `/framework-update` respectively; no request file generated | A request file generated for any of the three | 2026-09-04 |
| FW-INTAKE-005 | Mixed input keeps both halves | `/request` with "the price bug shipped AND the pipeline never caught it" | REQUEST_BUG for the app half AND the process half explicitly flagged for `/framework-update` | Either half silently dropped | 2026-09-04 |
...
| FW-ENH-001 | Stated fields bind; `unknown` fields become questions | `/enhance requests/<file>` where the file states the screen and marks WHY `unknown` | B3 asks about WHY (and only genuine unknowns); the stated screen is never re-asked or overridden | A stated field re-asked, or silently overridden by an assumption | 2026-09-04 |
...
**Delta this change:** added FW-INTAKE-001..006, FW-ENH-001..004; updated none; retired none.
