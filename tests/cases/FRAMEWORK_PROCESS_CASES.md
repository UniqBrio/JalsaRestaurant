...
| FW-INTAKE-005 | Mixed input keeps both halves | `/request` with "the price bug shipped AND the pipeline never caught it" | REQUEST_BUG written for the app half (stops for field review) AND the same run continues into workflows/framework-update.md with the process half | Either half silently dropped; the process half left as advice for the requester to run later | 2026-09-04 |
